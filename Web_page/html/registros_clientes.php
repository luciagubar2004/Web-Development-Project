<html>
    <head>
        <title>Conexion a la BD: Registro de Clientes (php)</title>
    </head>
    <body>
        <h3>Conexion a la BD: Registro de Clientes (php)</h3>
        <?php
            ...
            if (!$nombre || !$correo || !$apellido || !$fechnac || !$ciudad || !$contrasena)
            {
            ...
            ('". $correo ."', '". $nombre ."', '". $apellido ."', '". $contrasena ."', '" . $ciudad ."', '" . $fechnac ."', 2)"; 
           ...
           if ($resultado){
               echo "<script>
				alert('Cliente introducido correctamente');
			...
			echo  "<script>alert('Te has registrado correctamente');window.location.href='../raiz.php';</script>" ;
